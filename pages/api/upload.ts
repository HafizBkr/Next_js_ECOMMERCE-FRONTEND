  ...
  form.parse(req, async (err, fields, files) => {
    ...
    try {
      ...
      for (const file of fileArray) {
        ...
        const fileExtension = path.extname((file as any).originalFilename).toLowerCase();
        ...
        const filename = `${uuidv4()}-${(file as any).originalFilename}`;
        ...
        console.log('Fichier téléchargé avec succès sur Vercel Blob:', blob.url);
        urls.push(blob.url);
      }
      ...
      return res.status(500).json({ error: 'Erreur lors de l\'upload vers le storage' });
    }
  });
}
